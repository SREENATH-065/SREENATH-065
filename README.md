# Hi there, I'm Sreenath S! 👋

<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=2E9FFF&center=true&vCenter=true&width=600&lines=AI+%26+ML+Engineer;Full-Stack+Developer;Systems+%26+Distributed+Systems+Enthusiast;Hackathon+Builder+%26+Research+Explorer" alt="Typing SVG" />
</div>

---

## 🚀 About Me

I'm a Computer Science undergraduate specializing in **AI & Machine Learning** at NSS College of Engineering, Palakkad. I enjoy building scalable AI systems, distributed applications, and full-stack platforms that solve practical real-world problems.

* 🎓 **CGPA:** 9.14/10 (B.Tech CSE with Honors in AI & ML)
* 🤖 Exploring **LLMs, RAG Systems, Transformers & Edge AI**
* 🌐 Interested in **Distributed Systems, Full-Stack Engineering & Applied AI**
* 🏆 Active hackathon participant and collaborative builder
* 📫 Reach me at: **[sreenathssreenaths8@gmail.com](mailto:sreenathssreenaths8@gmail.com)**

---

# 🛠️ Tech Stack

### Programming Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge\&logo=python\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge\&logo=javascript\&logoColor=black)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge\&logo=java\&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge\&logo=kotlin\&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge\&logo=c%2B%2B\&logoColor=white)

---

### AI / ML & Data Science

![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge\&logo=tensorflow\&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge\&logo=scikit-learn\&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-000000?style=for-the-badge)
![FAISS](https://img.shields.io/badge/FAISS-009688?style=for-the-badge)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=for-the-badge)
![Whisper](https://img.shields.io/badge/Whisper-412991?style=for-the-badge)

---

### Full-Stack & Systems

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge\&logo=react\&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-43853D?style=for-the-badge\&logo=node.js\&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-005571?style=for-the-badge\&logo=fastapi)
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge\&logo=mongodb\&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge\&logo=firebase\&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge\&logo=git\&logoColor=white)

---

# 🧠 Research & Internship Experience

### 🔬 IIT Ropar — NPTEL Internship

Worked on **Niti-Setu**, a multilingual AI eligibility engine for government agricultural schemes using:

* React.js
* Node.js
* MongoDB
* LangChain
* RAG pipelines

Focused on document analysis and AI-based eligibility extraction from government scheme PDFs.

---

### 🧪 IIT Palakkad — Research Collaboration

Worked under **Prof. Mrinal Kanti Das** on:

* Transformer-based sentiment classification
* Custom multi-head attention mechanisms
* Bias analysis in LLMs
* Explainability and robustness evaluation

---

# 🏆 Featured Projects

---

## 📡 [HopGuard — BLE Mesh Communication App](https://github.com/SREENATH-065/HOPGUARD-GEC)

Offline peer-to-peer communication platform using Bluetooth Low Energy mesh networking for disaster-response scenarios.

### Key Features

* 📶 BLE mesh networking without internet/SIM
* 🗺️ GPS location sharing
* 🎙️ Voice notes + text communication
* 🔁 Decentralized offline message relay

### Tech Stack

`Kotlin` `Android SDK` `Bluetooth Low Energy`

---

## 🌾 [Niti-Setu — Voice-Based Scheme Eligibility Engine](https://github.com/Gayathrisubramanian06/Niti-setu)

AI-driven multilingual platform for government scheme eligibility analysis.

### Key Features

* 📄 Government scheme PDF ingestion
* 🔍 RAG-based eligibility extraction
* 🎤 Voice-based accessibility interaction
* 🌐 Multilingual workflow support

### Tech Stack

`React.js` `Node.js` `MongoDB` `LangChain`

---

## 💻 [CodeCollab — Real-Time Collaborative IDE](https://github.com/Gayathrisubramanian06/CodeCollab-)

Collaborative programming environment with real-time synchronization and debugging workflows.

### Key Features

* ⚡ Real-time collaborative editing
* 🔄 Yjs CRDT-based synchronization
* 🧠 Shared debugging workflows
* 🌐 Multi-user coding environment

### Tech Stack

`Next.js` `FastAPI` `Yjs` `Supabase`

---

## 📄 [PaperMentor — Research Exploration Platform](https://github.com/SREENATH-065/PaperMentor)

Semantic research assistant for PDF ingestion, contextual QA, and semantic retrieval.

### Key Features

* 📘 Research paper summarization
* 🔍 FAISS-based semantic search
* 💬 Context-aware Q&A
* 🌐 Web + PDF ingestion

### Tech Stack

`Python` `Streamlit` `FAISS` `LangChain`

---

## 🎥 [VidBrief — AI YouTube Summarization Platform](https://github.com/SREENATH-065/VidBrief)

AI-powered YouTube video understanding and summarization system.

### Key Features

* 🗣️ Whisper transcription pipeline
* 🧠 Semantic Q&A over transcripts
* 📝 Automated quiz generation
* ☁️ Streamlit deployment

### Tech Stack

`Python` `Whisper` `LangChain` `FAISS`

---

## 🚆 [RailTalk — Anonymous In-Train Chat App](https://github.com/SREENATH-065/RAILTALK)

Real-time anonymous communication platform for train passengers.

### Key Features

* 💬 Anonymous train-based communication
* 🧹 Auto-deletion after journey completion
* 📱 Mobile-friendly minimal interface
* 🏆 Finalist — RailHack 2025

### Tech Stack

`React` `Firebase` `Node.js`

---

# 🏅 Achievements

* 🥇 Winner — Hackathon, GEC Sreekrishnapuram (HopGuard)
* 🥇 Winner — CTF Competition, SOLASTA Event
* 🏆 Finalist — RailHack 2025 (Top 5 / 96 Teams)
* 🎓 NPTEL Elite Gold — Joy of Computing Using Python (Top 5%)
* 👨‍💻 Coordinator — Latent Space Learn & Hack Event
* 🚀 Co-Lead — HabitHack (FOSSERA Hackathon)

---

# 📜 Certifications

* 🤖 IBM — Machine Learning with Python
* ☁️ Udacity — AWS & Generative AI
* 🧠 IIT Ropar — Deep Learning
* 🚀 IBM SkillsBuild — AI & Cloud Internship

---

# 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=SREENATH-065&show_icons=true&theme=radical" />

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=SREENATH-065&theme=radical" />

<br/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SREENATH-065&layout=compact&theme=radical" />

</div>

---

# 🤝 Let's Connect!

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge\&logo=linkedin\&logoColor=white)](https://linkedin.com/in/sreenath-s-96a946290)

[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/SREENATH-065)

[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge\&logo=gmail\&logoColor=white)](mailto:sreenathssreenaths8@gmail.com)

</div>

---

<div align="center">

<img src="https://komarev.com/ghpvc/?username=SREENATH-065&color=brightgreen&style=flat-square" />

</div>

<div align="center">

⭐ From [SREENATH-065](https://github.com/SREENATH-065)

</div>
